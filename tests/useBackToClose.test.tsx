...
describe("언마운트 뒤에는 아무 이벤트도 안 받는다 (뒷정리)", () => {
  ...
  /* **리스너를 떼는 것만으로는 모자랍니다.** Escape는 모듈 스코프 스택에서 *가장 깊은
   * 것*을 임자로 고릅니다 — 죽은 항목이 남으면 그것이 계속 이기고 **다음에 열리는 팝업의
   * Escape가 아무 일도 안 하게** 됩니다. 정리에 `splice`가 있는 이유입니다.
   *
   * 📌 **이 자리는 이미 잡혀 있었습니다** — `splice`를 지우면 위쪽 겹침 검사 다섯이
   * 빨개집니다(재서 확인). 처음에 여기 *"위 검사는 그것을 안 본다"* 라고 썼는데 변이가
   * 그 문장을 반증했습니다. 그래도 남깁니다: 저 다섯은 **겹침 동작**을 재다 곁다리로
   * 잡는 것이고, 이 검사는 **뒷정리 계약을 이름으로** 말합니다 — 겹침 검사가 나중에
   * 모양을 바꿔도 이 계약은 남습니다. */
