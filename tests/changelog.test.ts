...
/* **`## 미출시`는 맨 위여야 합니다.**
 *
 * 실제로 밟았습니다(2026-08-13). 병렬 브랜치 둘이 이 파일에서 충돌했고, 해소하면서
 * `## 미출시`가 **`v0.6.0`과 `v0.5.0` 사이로** 들어갔습니다. 위쪽 검사들은 전부
 * `^## v`만 보므로 **하나도 안 잡았습니다.**
 *
 * 그리고 자리만 틀린 게 아닙니다 — 위 `releaseSections`의 분할도 `^## v` 기준이라
 * **미출시 본문이 `v0.6.0` 절 안으로 빨려 들어갑니다.** 즉 아직 안 나간 변경이
 * **이미 나간 릴리스의 일부로 읽힙니다.**
 *
 * 진짜 대가는 다음 릴리스에 옵니다: 그 제목을 `## v0.8.0`으로 바꾸는 순간
 * **v0.8.0이 v0.6.0과 v0.5.0 사이에 박힙니다.** 그때는 "맨 위 항목이 package.json의
 * 버전이다"까지 같이 깨지지만, 원인이 여기라는 것은 안 보입니다. */
describe("미출시 절은 맨 위에 있다", () => {
  const sectionTitles = [...changelogText.matchAll(/^## (.+)$/gm)].map((match) => match[1].trim());

  // 전제 — 제목을 하나도 못 읽었으면 아래가 공허하게 통과합니다.
  it("절 제목을 실제로 읽었다", () => {
    expect(sectionTitles.length).toBeGreaterThan(3);
    expect(sectionTitles.some((title) => /^v\d/.test(title))).toBe(true);
  });

  /* `indexOf`가 **없으면 `-1`**, **맨 위면 `0`**을 냅니다. 둘 다 정상이고(릴리스 직후에는
   * 미출시 절이 없는 것이 맞습니다), 끼어 있으면 `1` 이상이 되어 빨개집니다.
   * 조건 분기로 건너뛰지 않으므로 "미출시가 없어서 조용히 통과"가 안 생깁니다. */
  it("미출시 절이 릴리스 제목 뒤로 밀리지 않는다", () => {
    const ordered = sectionTitles.filter((title) => title === "미출시" || /^v\d/.test(title));
    expect(ordered.indexOf("미출시")).toBeLessThan(1);
  });
});
