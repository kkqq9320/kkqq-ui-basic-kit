...
불리므로 IME가 자기 일을 그대로 합니다. ⚠️ **이 인과는 아직 가설입니다** — 같은 캡처를
다시 떠서 확인할 항목으로 §10-12에 남깁니다.
...
10. ~~**IME 조합 중(`event.isComposing`)** — `shouldTrigger`가 안 봅니다.~~
    **✅ 닫혔습니다(2026-08-13).** 이제 규칙 8이 `isComposing`과 `keyCode === 229`를
    둘 다 봅니다 — §2.5.
12. **규칙 6의 `preventDefault`가 조합 중인 커서를 튀게 한 것이 맞는가.** 오너가 관측한
    증상(입력 커서가 맨 왼쪽으로)의 원인을 규칙 8이 없앴을 것으로 보지만 **그 인과는
    아직 가설입니다** — 데모의 "IME 확인" 패널로 같은 캡처를 다시 떠서 확인하세요.
