...
단축키입니다. **Windows에서 실기기로 확인했습니다(2026-08-09) — 브라우저가 가져가지
않고 페이지에 도달합니다. macOS `Cmd+;`는 아직 확인 전입니다.** 판정은
`event.code === "Semicolon"`입니다 — 배열에 따라 `;`가 Shift 조합이 되는 키보드가
있어 문자로 보면 새는 곳이 생깁니다.
...
- `vitest run` — 99/99 통과. 이 중 5개는 원본 `DateWheelPicker.test.tsx`를
