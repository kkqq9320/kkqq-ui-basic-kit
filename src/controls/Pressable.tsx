 ...
 * ## 여기 들어올 것과 안 들어올 것
 *
 * 🟢 **오너 결정 2026-08-23:** 휠 픽커의 `tapActivation`은 삼성 인터넷에서 `click`이
 * 사라진 것이 실측된 팝오버 액션에 둡니다. 이 경계는 계속 네이티브 `click`·키보드·
 * 프로그램 `click()`을 보존합니다. 같은 결함이 두 번째 컨트롤에서 실측되면 그때 이곳의
 * **명시적 opt-in 정책**을 검토합니다. 모든 버튼의 기본값을 `pointerup` 확정으로 바꾸는
 * 일은 순수 이동이 아니라 별도 입력 계약 이관입니다.
 *
