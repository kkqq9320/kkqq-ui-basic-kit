...
import { startsOnStepControl, useColumnSwipe } from "./columnSwipe";
...
export function WheelPicker({ model, value, onChange, min, max, fields, allowClear = false, step, ariaLabel, heading, id, disabled = false, labels: labelOverrides, timeZone = "Asia/Seoul", mobileBottomInset = 78, className = "" }: WheelPickerProps) {
  ...
  /* 스와이프의 기록과 그 수명은 `controls/columnSwipe.ts`가 소유합니다. 여기서 넘기는 것은
   * **이 컨트롤에서 무슨 뜻인가** 셋입니다: 한 칸 확정이 무엇인가, 제스처가 클릭을
   * 차지했다는 것을 어떻게 알리는가, 열들이 어디 사는가.
   *
   * ⚠️ 클릭 억제 표식은 위에 그대로 있습니다 — **열의 것**이고 스와이프는 알리기만 합니다. */
  const swipe = useColumnSwipe({
    active: open,
    containerRef: popoverRef,
    commit: (sourceValue, unit, amount, motion) => commitShift(sourceValue, unit, amount, motion),
    onGesture: () => { suppressColumnClickRef.current = true; },
    onGestureEnd: () => { releaseColumnClickSuppression(); },
  });
   ...
   * `"none"`이면 값만 확정합니다.
   *
   * **`"none"`을 쓰는 자리는 드래그 중 커밋 하나뿐입니다**(`controls/columnSwipe.ts`의 `move`). 근거는 그
   * 호출부에 적혀 있습니다.
   */
   ...
   * 값을 덮어써 방금 확정한 값이 사라집니다 — `columnSwipe`의 `move`/`end`가 이미
  ...
  return <div className={`wheel-picker ${className}`.trim()} data-open={open ? "true" : undefined} ref={rootRef}>
    ...
    <div className="wheel-trigger-shell"><Pressable id={id} buttonRef={triggerRef} className="wheel-trigger" data-editing={editing ? "true" : undefined} aria-label={triggerName} aria-haspopup="dialog" aria-expanded={open} aria-controls={open ? popoverId : undefined} disabled={disabled} onPaste={handlePaste} onFocus={() => { sessionStartValueRef.current = value; clearedRef.current = false; }} onBlur={() => { setEditing(false); flushTyping(); }} onClick={(event) => {
      ...
      <span className={[triggerParts ? "" : "placeholder", commitPulse ? "dropdown-value-commit" : ""].filter(Boolean).join(" ")} key={commitPulse}>{triggerParts
        ? triggerParts.map((part, index) => part.unit === null
          ...
          : <span className="wheel-segment" data-unit={part.unit} data-active={resolvedActiveUnit === part.unit ? "true" : undefined} key={part.unit}>{part.text}</span>)
    ...
    {open && position && createPortal(<div ref={popoverRef} id={popoverId} className="wheel-popover dropdown-menu-surface" role="dialog" aria-modal="false" aria-label={`${ariaLabel} ${labels.select}`} onMouseDown={(event) => event.preventDefault()} style={{ top: position.top, bottom: position.bottom, left: position.left, width: position.width, maxHeight: position.maxHeight }}>
      ...
      <div className="wheel-columns" data-fields={columns.length} style={{ "--wheel-rows": rowsPerSide } as CSSProperties}>
        ...
        {columns.map((unit) => { const motion = columnMotions.of(unit); const unitMark = model.columnMark?.(unit, fields) ?? ""; return <section className="wheel-column" aria-label={columnName(unit)} data-unit={unit} data-active={resolvedActiveUnit === unit ? "true" : undefined} data-entering={entering ? "true" : undefined} data-motion={motion.playing ? motion.direction : undefined} data-holding={hold.holdingUnit === unit ? "true" : undefined} role="group" style={{ "--wheel-unit-mark": JSON.stringify(unitMark) } as CSSProperties} onWheel={(event) => handleWheel(event, unit)} onPointerDown={(event) => { setTyping(null); if (!isPrimaryButton(event)) return; setActiveUnit(unit); suppressColumnClickRef.current = false; if (startsOnStepControl(event.target)) return; columnMotions.clear(unit); undo.beginGesture(baseValue); swipe.begin(unit, event.pointerId, event.clientY, baseValue); hold.arm(unit, event.pointerId, event.clientY); }} onPointerMove={(event) => { hold.cancelIfMoved(event.pointerId, event.clientY); swipe.move(unit, event.clientY, event.pointerId, event.buttons, event.currentTarget); }} onPointerUp={(event) => { hold.cancel(); swipe.end(unit, event.clientY, event.pointerId, event.currentTarget); undo.endGesture(); }} onPointerCancel={(event) => { hold.cancel(); undo.endGesture(); swipe.abort(event.currentTarget); }} onClickCapture={(event) => { if (suppressColumnClickRef.current) { event.preventDefault(); event.stopPropagation(); } }} key={unit}>
          ...
          <div className="wheel-viewport"><div className="wheel-values" key={`${unit}-${motion.sequence}`}>{offsets.map((offset) => { const rowValue = shifted(unit, offset); const preloadOnly = Math.abs(offset) === rowsPerSide + 1; const buffered = offset === 0 && resolvedTyping?.unit === unit ? resolvedTyping.digits : null; return <Pressable disabled={!rowValue} tabIndex={-1} aria-hidden={preloadOnly || undefined} aria-current={offset === 0 ? "date" : undefined} onClick={(event) => { if (offset === 0 && centreTapped(unit, event.timeStamp)) return; if (rowValue) applyShift(unit, offset); }} key={offset}>{buffered ?? (rowValue ? model.label(rowValue, unit, weekdayLabels, fields, hourDisplay) : "—")}</Pressable>; })}</div></div>
