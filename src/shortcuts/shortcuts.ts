 ...
 * 유일한 자리입니다. 빠뜨리면 **네이티브 편집이 죽고 액션만 돕니다**(규칙 6이
 * `preventDefault`를 부르므로). 그래도 목록으로 두는 것이 견디는 이유는 **범위**입니다 —
 * 이 구멍은 **앱이 그 조합을 실제로 바인딩했을 때만** 발현하고, 그건 앱이 압니다.
 * (전에 이 자리에 "조합이 두 번 동작해 눈에 보인다"고 적혀 있었는데 틀렸습니다 —
 * §10-7의 차단 모드 측정이 그것을 보여 줍니다.)
 *
 ...
 * 아무도 `preventDefault`를 안 부릅니다. **실패 모양도 같습니다**(규칙 6이 네이티브
 * 동작을 죽입니다).
 *
 * **다른 것은 범위입니다.** §2.3의 구멍은 **앱이 그 조합을 바인딩했을 때만** 발현하고
 * 그건 앱이 압니다. 여기는 **허용 구역 안 모든 폼 컨트롤이 자동으로** 걸렸습니다 —
 * 앱이 아무것도 잘못 안 해도 납니다. 그래서 이쪽은 목록이 아니라 규칙으로 닫았습니다.
 * (전에 이 자리에 "§2.3은 두 번 동작해 눈에 띄고 여기는 조용하다"고 적혀 있었는데,
 * 앞부분이 틀렸습니다 — §2.3의 실패도 네이티브 동작이 죽는 쪽입니다.)
 *
 ...
 * (`src/popupDismiss.ts`)·`Dialog`·`SectionTabs`·`PageChrome`도 같이 죽어서, 녹음 중 `Escape`로
