/* 가상 키보드가 열렸을 때 화면을 보정하는 두 훅 — **포커스된 것이 안 가려지게 밀어
 * 올리고, 닫힐 때 그 자리를 부드럽게 돌려준다.**
 *
 * `surfaces/AppShell.tsx`(1063줄)에서 갈라져 나왔습니다(PRINCIPLES §15 규칙 4).
 * 🔴 그 파일에서 **컴포넌트는 58줄이었고 나머지가 전부 이것**이었습니다 — 파일 이름이
 * 주제를 말하지 않고 있었습니다. 재 보니 셸과의 참조는 **훅을 부르는 세 줄뿐**이고,
 * 두 훅끼리는 상수·이징·`prefersReducedMotion` 넷을 공유해 한 파일이 맞습니다.
 *
 * `browser/`에 있는 이유: 같은 집안이 이미 여기 있습니다 —
 * `browser/visualViewport.ts`의 `useVirtualKeyboard`가 이 둘의 입력입니다.
 *
 * ⚠️ **`AppShell`이 상태 없이 도는 컴포넌트가 아닌 이유가 이 파일입니다.** 프롭은
 * controlled지만, 여기 있는 훅들이 `#root`를 **직접 스크롤하고** 닫히는 창구 동안
 * height까지 잠깐 붙듭니다. 소비 앱이 프롭으로 끄거나 대체할 수 없습니다.
 *
 * **절 번호 표기.** 이 파일에서 문서 이름 없이 쓰는 `§16.2`, `§14`, `§16.4`는 **Apple
 * 인터페이스 원칙**의 절 번호입니다(§16.2 Agency, §14 reduced-motion, §16.4 Familiarity).
 * 이 킷의 계약인 `PRINCIPLES.md`를 가리킬 때는 항상 문서 이름을 붙입니다.
 */
import { useLayoutEffect, useRef, useState } from "react";

import { type VirtualKeyboard } from "./visualViewport";
 ...
 * 자체 키보드 보정을 갖추기 전부터 조상 저장소의 베이스라인에 주석·커밋 메시지 설명
 ...
 */
export function useKeyboardScrollCompensation(keyboard: VirtualKeyboard, scrollRootId = "root") {
 ...
 */
export function useReleasableKeyboardInset(keyboard: VirtualKeyboard, scrollRootId = "root"): number {
