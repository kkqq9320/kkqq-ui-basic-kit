...
이 키트는 **의존성으로 받아서 프로젝트마다 갈아 끼워** 씁니다. 색·연월 픽커처럼
프로젝트마다 달라지는 것들은 대부분 **키트(GitHub)를 고치지 않고** 프롭이나 CSS
...
| **3. 키트 수정** | 마운트 id, 배포 폰트, 빠른 바 칸 수, 없는 프롭/컴포넌트 | ✅ 이 저장소에서 고쳐 push |
...
| `labels` | 한국어 | 다른 언어. `placeholder`·`hint`·`today`·`clear`·`done`·`setToday`·`previous`·`next`·`select`·`weekdays[7]`·`units{year,month,day}` 중 **필요한 키만** |
