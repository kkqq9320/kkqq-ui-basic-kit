...
| `labels` | 한국어 | 다른 언어. 최상위 키는 **필요한 것만** 주면 됩니다. ⚠️ **`units`만 예외로 여섯을 다 줘야 합니다**(`year`·`month`·`day`·`hour`·`minute`·`second`) — 세 개만 주면 컴파일이 거절합니다. 일부러입니다: 이 병합은 키마다 펼쳐지므로, 셋만 주면 나머지 셋이 **한국어로 남아 스크린리더에 나갑니다**(화면에는 안 보입니다). `hint`는 팝오버 머리말에만 나옵니다 — 키 조작은 팝오버를 열지 않아도 되므로 그 문구가 조작의 전부를 안내하지는 않습니다 |
...
반응형 그리드 넷이 각각 **세 축**을 토큰으로 엽니다. 기본값은 전부 "지금과 같음"이라
아무것도 안 하면 화면이 안 바뀝니다.

| 그리드 | 최소 | 상한 | 남는 폭 |
|---|---|---|---|
| 요약 카드 | `--summary-card-min`(240px) | `--summary-card-max`(1fr) | `--summary-card-justify`(normal) |
| 패널 | `--panel-min`(400px) | `--panel-max`(1fr) | `--panel-justify`(normal) |
| 패널 안 필드 | `--field-min`(230px) | `--field-max`(1fr) | `--field-justify`(normal) |
| 색상 카드 | `--color-card-min`(240px) | `--color-card-max`(1fr) | `--color-card-justify`(normal) |
...
길이를 주는 순간 칸 수를 세는 기준이 `min`에서 `max`로 바뀝니다:

```
열 수 = floor((폭 + gap) / (max + gap))
```

그래서 **`max`를 올리면 열이 줄고 카드는 오히려 커집니다.** 실측(작업 영역 908px, gap 14):

| `--summary-card-max` | 열 수 | 카드 폭 |
|---|---|---|
| 없음(`1fr`) | 5 | 170.4px |
| `210px` | 4 | 210px |
| **`300px`** | **2** | 300px |
...
그리고 **칸은 언제나 정확히 `max`입니다** — `min`과 `max` 사이 어딘가로 눕지 않습니다.
남는 폭은 전부 줄 끝에 몰립니다.

#### ⚠️ `justify`는 `max`가 있어야 동작합니다

`max`가 `1fr`이면 트랙이 남는 폭을 이미 다 나눠 가져 **옮길 폭이 0**입니다. 실측으로
`normal`·`start`·`space-between`·`space-around`·`space-evenly` 다섯이 **좌표까지
완전히 같았습니다.** 그리고 항목이 없어 비어 있는 **칸**은 남는 **폭**이 아니라서
이 축이 못 건드립니다.

> `normal`은 그리드에서 `start`로 동작합니다(실측 좌표 동일) — `start`를 따로 쓸
> 이유가 없습니다.

#### 쓰는 순서

1. **`min`으로 "한 칸이 이보다 좁으면 안 된다"를 정합니다.** 대개 여기서 끝납니다.
2. 넓은 화면에서 칸이 너무 커지면 **`max`로 상한**을 줍니다. 이때 열 수가 위 공식대로
   바뀌므로 **좁은 화면도 같이 보세요** — 큰 값은 좁은 화면에서 열을 뺍니다.
3. `max`를 준 순간 생긴 남는 폭을 **`justify`로 배치**합니다. 줄 끝을 다른 블록과
   맞추고 싶으면 `space-between`이나 `end`입니다.

```css
:root {
  --summary-card-min: 200px;
  --summary-card-max: 260px;        /* 여기서부터 남는 폭이 생깁니다 */
  --summary-card-justify: space-between;   /* 줄 끝이 아래 패널과 맞습니다 */
}
```

일반 규칙(호스트 안에 얹는 화면 포함)은
[`LAYOUT-PRINCIPLES.md`](LAYOUT-PRINCIPLES.md) §2에 있습니다.
